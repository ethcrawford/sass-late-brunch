...
class SassCompiler {
      ...
      libsass.render(
        ...
        (error, result) => {
          ...
          const data = result.css
            .toString()
            .replace("/*# sourceMappingURL=a.css.map */", "");
  ...
  _rubyCompile(source) {
    ...
    return this.rubyPromise.then(() => {
      var debugMode = this.config.debug,
        hasComments;
      if (
        (debugMode === "comments" || debugMode === "debug") &&
        !this.optimize
      ) {
      ...
      if (this.config.precision)
        cmd.push(`--precision=${this.config.precision}`);
  ...
  compile(params) {
    ...
    return this.seekCompass(path, data)
      .then(imports => {
        const source = {
          data,
          path,
